...
// Shoelace formula -- the region's real area, as opposed to its bounding
// box's area. A long, irregular, coastline-hugging region (a forest strip
// following a winding shore, say) can have a bounding box many times
// larger than its actual footprint; sizing the texture-mask circle count
// off the bounding box in that case scatters far more circles than the
// shape needs, most landing outside it anyway once the clip below is
// applied -- wasted draw calls for no visual benefit.
...
// `boundary` is either a single loop (array of points, the original form --
// need not already be closed, an open coastline-style chain is closed by
// connecting its last point back to its first) or `{outer, holes}` (an
// outer loop plus zero or more interior holes, e.g. a lake enclosed by a
// mountain mass -- both loops must already be closed, which is
// closeContourChains's job, not this function's). Multi-loop mode fills and
// clips with the evenodd rule across outer+holes together in one path, so a
// hole is correctly left unpainted regardless of either loop's winding
// direction -- no separate "is this a hole" classification needed beyond
// grouping them together in the first place (terrain-grid.js's
// groupChainsIntoLoops). `hsl` is the wash's base {h, s, l} tone (see
...
function paintWatercolorWash(ctx, boundary, rng, hsl, ink, layers) {
  const loops = Array.isArray(boundary) && boundary.length && !Array.isArray(boundary[0]) && boundary[0].x !== undefined
    ? { outer: boundary, holes: [] }
    : boundary;
  const outer = loops.outer, holes = loops.holes || [];
  if (outer.length < 3) return;
  layers = layers || 25;
  const bounds = polygonBounds(outer);
  const span = Math.max(1, Math.min(bounds.width, bounds.height));
  const multi = holes.length > 0;

  function fillLoops(loopList) {
    ctx.beginPath();
    ...
    ctx.fill(multi ? 'evenodd' : 'nonzero');
  }
  function clipLoops(loopList) {
    ctx.beginPath();
    for (const loop of loopList) {
      ctx.moveTo(loop[0].x, loop[0].y);
      for (let i = 1; i < loop.length; i++) ctx.lineTo(loop[i].x, loop[i].y);
    ...
    ctx.clip(multi ? 'evenodd' : 'nonzero');
  }
  ...
  fillLoops([outer, ...holes]);

  // Everything from here on is clipped to the true (unjittered) boundary --
  // a long, irregular, coastline-hugging region can have a bounding box far
  // larger than its actual footprint, and without this clip the texture-
  // mask circle scatter below (positioned within that bounding box) lands
  // visibly outside the shape: reported directly against a live map, where
  // it showed up as translucent grey "fog" blobs scattered out at sea, far
  // from any actual hill/forest region. In multi-loop mode the same clip
  // also keeps the wash off any interior hole (a lake inside a mountain
  // mass), for the same reason the fill above does.
  ctx.save();
  clipLoops([outer, ...holes]);

  const variance = Math.max(2, span * 0.05);
  for (let i = 0; i < layers; i++) {
    const jittered = jitterPolygon(outer, rng, variance, 4);
  ...
  // Sized off the polygon's real area (not its bounding box -- see
  // polygonArea's comment) so an elongated region gets circle density
  // proportional to how much shape there actually is to texture, not to
  // how much empty bounding-box space surrounds it.
  // Capped -- circle count otherwise scales unbounded with painted area, and
  // a continent-scale canvas (several times the area this was tuned
  // against) would multiply this pass's cost well past what the visual
  // texture actually needs once a region is already large enough to be
  // covered many times over.
  const circleCount = Math.min(5000, Math.round(polygonArea(outer) / 70));
  ...
  ctx.globalAlpha = 0.5;
  ctx.lineWidth = 1.2;
  strokePolygonPath(ctx, outer);
  ctx.stroke();
  for (const hole of holes) {
    strokePolygonPath(ctx, hole);
    ctx.stroke();
  }
  ctx.globalAlpha = 1;
}
