...
// A single LOCALIZED mountain range: samples `ridgedSampler` (a
// makeRidgedFbmSampler instance the caller already built with its own rng)
// through a rotated ellipse centered at (cx, cy) in canvas-PIXEL
// coordinates (not normalized [0,1] -- so multiple ranges with different
// centers/sizes can be combined directly by a caller without each one
// renormalizing against canvas dimensions itself). Returns 0 outside the
// ellipse, ridged-noise * a smoothstep falloff inside.
//
// This envelope is the piece a single global range never needed: today's
// one-range code (makeAnisotropicSampler applied to the whole canvas) has
// no boundary at all, which is fine for exactly one range but means several
// of them would smear into one indistinguishable mass with nothing to
// separate them. `lengthRadius`/`widthRadius` are already the SOURCE of
// this range's anisotropy (an elongated ellipse reads as a range with a
// real long axis), so this does not need makeAnisotropicSampler's own
// stretch -- the ellipse's own aspect ratio does that job.
function makeMountainRange(ridgedSampler, cx, cy, angle, lengthRadius, widthRadius) {
  const cos = Math.cos(angle), sin = Math.sin(angle);
  return function (x, y) {
    const dx = x - cx, dy = y - cy;
    const nu = (dx * cos + dy * sin) / lengthRadius;
    const nv = (-dx * sin + dy * cos) / widthRadius;
    const d2 = nu * nu + nv * nv;
    if (d2 >= 1) return 0;
    const envelope = smoothstep(1 - d2);
    return ridgedSampler(0.5 + nu * 0.5, 0.5 + nv * 0.5) * envelope;
  };
}

// A bare smooth elliptical falloff (1 at center, fading to 0 at the ellipse
// boundary) -- the same envelope math as makeMountainRange, minus the
// noise sampling. Meant to be combined via max() across several instances
// to build a LANDMASS as a union of elongated "lobes" instead of one
// radial shape: lobes that fully overlap read as one connected body, lobes
// that only just touch read as a peninsula on a narrow neck -- real
// coastline complexity (a deep interior sea, a sub-peninsula on an
// isthmus) that a single wobbled circle can't produce no matter how much
// wobble amplitude you add to it. Confirmed against actual reference maps
// (Faerun, Middle-earth, Westeros), not assumed.
function makeLobeFalloff(cx, cy, angle, lengthRadius, widthRadius) {
  const cos = Math.cos(angle), sin = Math.sin(angle);
  return function (x, y) {
    const dx = x - cx, dy = y - cy;
    const nu = (dx * cos + dy * sin) / lengthRadius;
    const nv = (-dx * sin + dy * cos) / widthRadius;
    const t2 = nu * nu + nv * nv;
    return Math.max(0, 1 - t2 * 1.3);
  };
}
