...
function paintHachureField(ctx, w, h, rng, ink, heightAt, skipAt, opts) {
  ...
  // Coverage is modulated by a coarse noise field rather than applied at
  // an even rate everywhere. At uniform density this pass reads as a
  // solid mat of straw -- one flat texture wall to wall, with no bare
  // ground, no open patches and nothing for the eye to rest on, which is
  // both unlike the reference (whose hachure thins and thickens across a
  // slope) and actively hides the buildings and streets sitting on it.
  // Gating on a smooth field instead makes the sparse and dense areas
  // COHERENT patches rather than per-stroke static.
  const patch = makeFbmSampler(rng, 3);
  const patchFloor = opts.patchFloor !== undefined ? opts.patchFloor : 0.42;
  ...
  for (let pass = 0; pass < passes; pass++) {
    for (let gy = spacing / 2; gy < h; gy += spacing) {
      for (let gx = spacing / 2; gx < w; gx += spacing) {
        ...
        // Sampled well below 1:1 so the patches are large features, not
        // per-stroke noise.
        const cover = patch(px / w * 3, py / h * 3);
        if (cover < patchFloor) continue;
        ...
        // Alpha rides the same field, so a patch fades in and out at its
        // edges instead of stopping at a hard line.
        const strength = Math.min(1, (cover - patchFloor) / 0.35);
        ctx.globalAlpha = (0.18 + rng() * 0.32) * (0.35 + 0.65 * strength);
