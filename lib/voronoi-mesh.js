...
// Walks every land cell's polygon edges and keeps the ones that border
// water, returning them as ordered polylines (open or closed) rather than a
// disconnected bag of segments -- Chaikin smoothing (below) needs a walkable
// sequence of points, not just "here are some coastal line segments".
//
// Which neighbor a given polygon edge borders is found geometrically rather
// than tracked separately: a Voronoi edge's midpoint is, by construction,
// closer to its own two defining sites than to any other site in the mesh,
// so "which of this cell's listed neighbors has the site nearest this
// edge's midpoint" reliably recovers the bordering cell without needing the
// underlying triangulation data (which isn't retained on the returned
// mesh). A polygon edge from canvas-boundary clipping (not a real
// Voronoi-neighbor edge) has no true bordering site, so this heuristic can
// occasionally misattribute one such edge at the map's outer border -- low
// stakes, since that's the one place the map itself is already truncated.
function extractCoastlineChains(cells, isLand) {
  const segments = [];
  for (const cell of cells) {
    if (!isLand(cell.index)) continue;
    ...
    for (let i = 0; i < poly.length; i++) {
      ...
      for (const nbIdx of cell.neighbors) {
        ...
        const d = (nb.x - midX) * (nb.x - midX) + (nb.y - midY) * (nb.y - midY);
        if (d < bestDist) { bestDist = d; bestNb = nbIdx; }
      }
      if (bestNb !== -1 && !isLand(bestNb)) segments.push([p1, p2]);
    }
  }
...
function chainSegments(segments) {
  const GRID = 0.05; // finer than any real vertex spacing, so shared Voronoi vertices collapse to the same key despite float roundoff
  const key = (p) => Math.round(p.x / GRID) + '_' + Math.round(p.y / GRID);
  const pointMap = new Map();
  const addPoint = (p, segIdx, which) => {
    const k = key(p);
    if (!pointMap.has(k)) pointMap.set(k, []);
    pointMap.get(k).push({ segIdx, which });
  };
  segments.forEach((seg, i) => { addPoint(seg[0], i, 0); addPoint(seg[1], i, 1); });
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    while (extended) {
      ...
      for (const c of pointMap.get(key(tail)) || []) {
        if (used[c.segIdx]) continue;
        const seg = segments[c.segIdx];
        chain.push(c.which === 0 ? seg[1] : seg[0]);
        used[c.segIdx] = true;
        extended = true;
        break;
      }
    ...
    while (extended) {
      extended = false;
      const head = chain[0];
      for (const c of pointMap.get(key(head)) || []) {
        if (used[c.segIdx]) continue;
        const seg = segments[c.segIdx];
        chain.unshift(c.which === 0 ? seg[1] : seg[0]);
        used[c.segIdx] = true;
        extended = true;
        break;
      }
...
// at 1/4 and 3/4 along it, pulling the line toward a smooth curve. Pure
// deterministic geometry over already-generated points -- no rng, no
// seed-reproducibility risk at all, unlike nearly everything else in this
// generator. The original first/last points are kept on every iteration
// (the open-curve variant) so a coastline chain doesn't visibly shrink away
// from its own endpoints as iterations increase.
function chaikinSmooth(points, iterations) {
  ...
  for (let iter = 0; iter < iterations; iter++) {
    ...
    for (let i = 0; i < pts.length - 1; i++) {
      ...
      next.push({ x: p0.x + (p1.x - p0.x) * 0.25, y: p0.y + (p1.y - p0.y) * 0.25 });
      next.push({ x: p0.x + (p1.x - p0.x) * 0.75, y: p0.y + (p1.y - p0.y) * 0.75 });
    }
...
// Closed-loop counterpart to chaikinSmooth above -- treats the point list as
// cyclic (wraps from the last point back to the first) instead of pinning
// the first/last points, so smoothing a closed coastline/biome-band loop
// doesn't leave one sharp, unsmoothed corner wherever marching squares
// happened to start tracing it. Used on the raw extractFillableRegions
// loops themselves (map-overworld.js/map-detail.js's pathFromLoops) --
// those loops' own STROKE was already smoothed (see the coastline-emphasis
// pass), but the FILLED shape underneath was still the raw, blocky marching-
// squares polygon, which is what actually reads as jagged since it's a
// large filled area rather than a thin line.
